...
class Projects(webapp2.RequestHandler):
  ...
  def put(self, project_id):
    ...
    if (not project.is_owner(user.email)
      and not project.has_contributor(user.email)):
      self.abort(401)
    # Process changes...
    name = request_object.get('name')
    if name:
...
class TimeRecords(webapp2.RequestHandler):
  ...
  def put(self, project_id, time_record_id):
    ...
    if (not (project and isinstance(project, model.Project))
      or not (time_record and isinstance(time_record, model.TimeRecord))):
      self.abort(404)
    if ((user.email not in project.contributors)
      and not project.is_owner(user.email)):
      self.abort(401)
...
class Comments(webapp2.RequestHandler):
  def get(self, project_id, parent_id=None):
    ...
    if parent_id:
      ...
      if not (parent and (isinstance(parent, model.Milestone)
        or isinstance(parent, model.TimeRecord))):
        self.abort(404)
  ...
  def put(self, project_id, comment_id):
    ...
    if (not project_key
      or not comment_key
      or not comment_content
      or (project_key not in comment_key.parent())): # TODO: Test this!
      self.abort(400)
    project = project_key.get()
    comment = comment_key.get()
    if (not (project and isinstance(project, model.Project))
      or not (comment and isinstance(comment, model.Comment))):
      self.abort(404)
    if ((user.email not in project.contributors)
      and not project.is_owner(user.email)):
      self.abort(401)
...
class Milestones(webapp2.RequestHandler):
  ...
  def put(self, project_id, milestone_id):
    ...
    request_object = json.loads(self.request.body)
    if (not project_key
      or not milestone_key
      or (project_key != milestone_key.parent())):
      self.abort(400)
    project = project_key.get()
    milestone = milestone_key.get()
    if (not (project and isinstance(project, model.Project))
      or not (milestone and isinstance(milestone, model.Milestone))):
      self.abort(404)
    if user.email not in project.users:
      self.abort(401)
    # Process optional items...
  ...
  def delete(self, project_id, milestone_id):
    ...
    if (not project_key
      or not milestone_key
      or (project_key != milestone_key.parnet())):
      self.abort(400)
    project = project_key.get()
    milestone = milestone_key.get()
    if (not (project and isinstance(project, model.Project))
